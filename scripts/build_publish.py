...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="beskriva-verksamheten-epub-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="beskriva-verksamheten-pdf-") as tmp_name:
        tmp = Path(tmp_name)
